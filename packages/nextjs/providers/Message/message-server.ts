import { PartyKitExtended, PartyKitServerWithMoreFun } from "~~/party/extended";
...
export default {
  ...
  async onConnectExtended(conn, room) {
    const messages = (await room.storage.get<ChatMessage[]>(Storage.messages)) ?? [];
  ...
  async onCloseExtended(conn, room) {
    ...
    const usersTyping = (await room.storage.get<string[]>(Storage.usersTyping)) ?? [];
  ...
  async onMessageExtended(msg, conn, room) {
    ...
    if (parsedMsg.newMessage) {
      const messages: ChatMessage[] = (await room.storage.get(Storage.messages)) ?? [];
    ...
    if (parsedMsg.isTyping) {
      const isTyping = (await room.storage.get<string[]>(Storage.usersTyping)) ?? [];
      const currentUsersTyping = [...new Set([...isTyping, parsedMsg.isTyping])];
