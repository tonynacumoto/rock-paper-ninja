...
export default function ConnectionStatus(props: { socket: PartySocket | WebSocket | null }) {
  const { socket } = props;
  const [readyState, setReadyState] = useState<number>(socket?.readyState === 1 ? 1 : 0);
